...
static void IRAM_ATTR wifi_sniffer_cb(uint8 *recv_buf, uint16 len)
{
  static uint32_t index, ptr;
  ...
  {
    unsigned int sig_len = sniffer->rx_ctrl.sig_mode == 0 ? sniffer->rx_ctrl.legacy_length : sniffer->rx_ctrl.HT_length;
    ...
    while (index < (sig_len - 4))
    {
      ...
      if (tlv->type == 0xDD)
      {
        ...
        {
          ptr = 0;
          while (ptr < ((uint32_t)gse_vendor->length - 4))
          {
